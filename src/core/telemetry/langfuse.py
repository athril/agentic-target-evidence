...
#     """Async context manager that wraps an operation in an OTel span.

#     Sets ``langfuse.trace_id`` so Langfuse can correlate spans back to a run.
#     Records wall-clock latency and any exception on exit.
#     """
#     tracer = get_tracer()
#     with tracer.start_as_current_span(name) as current_span:
#         current_span.set_attribute("langfuse.trace_id", trace_id)
#         # Mark as a GenAI span so Langfuse's default should_export_span filter includes it.
#         current_span.set_attribute("gen_ai.system", "gene-target-validation")
#         if attributes:
...
#     model: str,
# ) -> None:
#     """Attach token-cost attributes that Langfuse reads for cost tracking."""
